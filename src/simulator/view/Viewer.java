...
class Viewer extends SimulationViewer {
	...
	private void showHelp(Graphics2D g) {
		...
		Font stringFont = new Font( "Arial", Font.PLAIN, 14 );
		g.setFont(stringFont);
		
		g.drawString("h: toggle help, v: toggle vectors, +: zoom-in, -: zoom-out, =: fit", 5, 15);
		g.drawString("g: show next group", 5, 35);
		g.drawString("l: move right, j: move left, i: move up, m: move down: k: reset ", 5, 55);
		g.drawString("Scaling ratio: " + _scale, 5, 75);
		
		g.setColor(Color.BLUE);
		
		if(_selectedGroupIdx != -1)
			g.drawString("Selected Group: " + _selectedGroup, 5, 95);
		else
			g.drawString("Selected Group: all" , 5, 95);
	}
	...
	private void drawBodies(Graphics2D g) {
		...
		for(Body b : _bodies) {
			...
			if(isVisible(b)) {
				...
				double vx = (b.getVelocity().direction()).getX()*25 + b.getPosition().getX()/_scale + _centerX;
				double vy = (b.getVelocity().direction()).getY()*25 + b.getPosition().getY()/_scale +  _centerY;
				double fx = (b.getForce().direction()).getX()*25 + b.getPosition().getX()/_scale + _centerX;
				double fy = (b.getForce().direction()).getY()*25 + b.getPosition().getY()/_scale + _centerY;
				
				if(_showVectors) {
					drawLineWithArrow(g, (int)(b.getPosition().getX()/_scale) + _centerX + 3, (int)(b.getPosition().getY()/_scale) + _centerY + 4, 
									  (int)vx, (int)vy, 5, 3, Color.RED, Color.RED);
					drawLineWithArrow(g, (int)(b.getPosition().getX()/_scale) + _centerX + 3, (int)(b.getPosition().getY()/_scale) + _centerY + 4, 
							  (int)fx, (int)fy, 5, 3, Color.GREEN, Color.GREEN);
				}
	
				g.setColor(Color.BLACK);
				g.drawString(b.getId(), (int)(b.getPosition().getX()/_scale) + _centerX, (int)(b.getPosition().getY()/_scale) + _centerY - 5);
				
				g.setColor(_gColor.get(b.getgId()));
				g.fillOval((int)(b.getPosition().getX()/_scale) + _centerX, (int)(b.getPosition().getY()/_scale) + _centerY, 10, 10);
			}
		}
	}
